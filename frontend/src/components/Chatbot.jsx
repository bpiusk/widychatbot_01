...
export default function Chatbot() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-white via-pink-100 to-purple-100">
      {/* Header */}
      <header className="w-full flex flex-col items-center pt-8 pb-4 px-0 md:px-4">
        ...
        <h1 className="text-2xl md:text-3xl font-light text-gray-800 text-center font-[Manrope]">
      ...
      <main className="flex-1 w-full flex justify-center">
        {/* Pastikan px-0 diterapkan di sini untuk mobile, md:px-4 untuk desktop */}
        <div className="w-full max-w-2xl flex flex-col px-0 md:px-4">
          <div className="flex-1 flex flex-col gap-4 overflow-y-auto pb-16">
            {messages.length === 0 && (
            ...
            {messages.map((msg, idx) => (
              ...
              >
                {/* Perbaikan ukuran font di sini: text-lg sm:text-base */}
                <div
                  className={`px-4 py-3 max-w-[80%] rounded-2xl text-lg sm:text-base shadow-md break-words ${
                    msg.from === "user"
      ...
      {/* Footer */}
      {/* Perbaikan ukuran font di sini: text-sm sm:text-xs */}
      <footer className="w-full text-center text-sm sm:text-xs text-gray-500 py-2 bg-transparent">
      ...
      <form
        onSubmit={handleSend}
        // Pastikan px-0 diterapkan di sini untuk mobile, md:px-0 untuk desktop (jika ingin tetap edge-to-edge)
        className="w-full fixed bottom-0 left-0 flex justify-center bg-transparent px-0 py-3 md:px-0"
        autoComplete="off"
      >
        <div className="w-full max-w-2xl flex items-center bg-white border border-gray-200 rounded-full shadow-lg px-4 py-2 gap-2">
          ...
          <input
            ...
            disabled={loading}
            className="flex-1 bg-transparent border-none outline-none text-lg sm:text-base px-2 py-2"
          />
          <button
            type="submit"
